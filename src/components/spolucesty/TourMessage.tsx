  ...
  {replies.sort((a, b) => b.id - a.id).map(reply => {
    ...
    if (reply.message_id === tourMessage.id) {
      ...
      return (
        ...
        <span className='absolute text-xs left-16 top-0'>{reply.messageType == 1 ? `Tato zpáva je zašifrovaná a vidí ji pouze uživatel ${user?.firstName} a ${tourMessage.firstName} `:''}</span>
          <div className={`flex items-center gap-6 md:gap-2  cursor-pointer mt-1 ${reply.user_id ===  user?.id ? 'pl-1': 'p3-6' }`}>
            {reply.user_id === user?.id &&
              <div className="text-red-700 hover:text-red-500 absolute top-1 right-1 " onClick={() => handleDeleteClick(reply.id)}>
